...
def test_needs_pose_control_for_non_hdr_with_video():
    assert _needs_pose_control_preprocessing([("/loras/union.safetensors", 1.0)], [("m.mp4", 1.0)])


def test_needs_pose_control_false_for_hdr():
    with patch("ltx_mlx_backend._ic_lora_uses_hdr_pipeline", return_value=True):
        assert not _needs_pose_control_preprocessing([("/loras/hdr.safetensors", 1.0)], [("m.mp4", 1.0)])
