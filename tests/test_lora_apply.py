...
def test_count_lora_weight_matches():
    from ltx_mlx_backend import _count_lora_weight_matches
    ...
    lora = {
        ...
        "transformer_blocks.0.attn1.to_v.lora_A.weight",
        "transformer_blocks.0.attn1.to_v.lora_B.weight",
    }
    matched, total = _count_lora_weight_matches(model, lora)
    assert total == 2
    assert matched == 1


def test_should_keep_ic_lora_for_stage2_crossview():
    from types import SimpleNamespace

    from ltx_mlx_backend import _should_keep_ic_lora_for_stage2
    ...
    assert _should_keep_ic_lora_for_stage2(pipe) is True


def test_should_keep_ic_lora_for_stage2_skips_hdr_path():
    from types import SimpleNamespace

    from ltx_mlx_backend import _should_keep_ic_lora_for_stage2

    pipe = SimpleNamespace(
        _lora_paths=[("/weights/LTX-2.3-22b-IC-LoRA-HDR.safetensors", 1.0)]
    )
    assert _should_keep_ic_lora_for_stage2(pipe) is False


def test_should_keep_ic_lora_for_stage2_skips_empty():
    from types import SimpleNamespace

    from ltx_mlx_backend import _should_keep_ic_lora_for_stage2

    assert _should_keep_ic_lora_for_stage2(SimpleNamespace(_lora_paths=[])) is False
    assert _should_keep_ic_lora_for_stage2(SimpleNamespace()) is False
