...
- **`chain_method: native_extend`**: clip 1 is `generate`/i2v; clips 2+ run **`mode: extend`** with the prior clip MP4 as `source_video` (native ltx-2-mlx `RetakePipeline.extend_from_video`). Set `extend_frames` (latent count, default 2). Incompatible with `audiocontinue`.
